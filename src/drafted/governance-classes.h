 ...
 *
 *   // network
 *   CCupcoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.cupcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CCupcoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CCupcoinNetwork(UniValue objIn)
    {
...
// // can be under: CupcoinNetwork
// //   -- signature requirements : Key1(User)
// class CCupcoinNetworkVariable : public CGovernanceObject
// {
// private:
