...
echo "**********************************************************************"
echo "*                                                                    *"
echo "* This script will install and configure your CupCoin masternode.  *"
echo "*                                                                    *"
...
read -p "This script will install & configure a CupCoin Masternode.  Do you wish to continue? (y/n)? " response
...
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
	...
	# Download CupCoin
	echo && echo "Downloading CupCoin.."
	sleep 3
	cd
	git clone https://github.com/cupcoinChain/Cupcoin.git
	
	# Install CupCoin
	echo && echo "Installing CupCoin..."
	sleep 3
	cd Cupcoin
	...
	# Create config for CupCoin
	echo && echo "Configuring CupCoin..."
	sleep 3
	cd ~
	cd Cupcoin/src
	./cupcoind -daemon
	...
	cd /root/.cupcoincore 
	echo -e "rpcuser=${rpcuser}\nrpcpassword=${rpcpass}\nrpcallowip=127.0.0.1\nrpcport=11998\nrpcthreads=8\nlisten=1\nserver=1\ndaemon=1\nstaking=0\ndiscover=1\nexternalip=${ip}:12875\nmasternode=1\nmasternodeprivkey=${key}\naddnode=84.17.23.43:12875\naddnode=18.220.138.90:12875\naddnode=86.57.164.166:12875\naddnode=86.57.164.146:12875\naddnode=18.217.78.145:12875\naddnode=23.92.30.230:12875\naddnode=35.190.182.68:12875\naddnode=80.209.236.4:12875\naddnode=91.201.40.89:12875" > /root/.cupcoincore/cupcoin.conf
	
	# Download and install sentinel
	echo && echo "Installing Sentinel..."
	sleep 3
	cd /root/.cupcoincore
	git clone https://github.com/cupcoinChain/sentinelLinux.git && cd sentinelLinux
	export LC_ALL=C
	virtualenv ./venv
	./venv/bin/pip install -r requirements.txt
	echo  "* * * * * cd /root/.cupcoincore/sentinelLinux && ./venv/bin/python bin/sentinel.py >/dev/null 2>&1" >> mycron
	crontab mycron
	rm mycron
	rpl dash_conf=/home/YOURUSERNAME/.cupcoincore/cupcoin.conf dash_conf=/root/.cupcoincore/cupcoin.conf sentinel.conf
	
	#Start CupCoin Daemon
	sleep 60
	cd /root/Cupcoin/src
	./cupcoind -daemon
	cd ~
	echo && echo && echo
	echo "CupCoin installation completed successfully.  Please wait 15 minutes and then start your masternode from your local wallet"	
else
