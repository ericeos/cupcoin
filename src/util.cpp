...
const char * const BITCOIN_CONF_FILENAME = "cupcoin.conf";
const char * const BITCOIN_PID_FILENAME = "cupcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cupcoin" is a composite category enabling all Cupcoin-related debug output
            if(ptrCategory->count(string("cupcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CupcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CupcoinCore
    // Mac: ~/Library/Application Support/CupcoinCore
    // Unix: ~/.cupcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CupcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CupcoinCore";
#else
    // Unix
    return pathRet / ".cupcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cupcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cupcoin.conf
